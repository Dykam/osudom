...
namespace TreeCore
{
	...
	public class LinkedNodeList : INodeList
	{
		...
		public virtual int InsertAt(int position, INode node)
		{
			if (initFirst(node))
			{
				if (position > firstNode.Lenght)
				{
					firstNode.Prepend(node);
					return firstNode.Lenght - 1;
				}
				else
				{
		...
		public virtual int InsertAt(int position, INodeList nodes)
		{
			...
			LinkedListNode<INode> nodeListToAtTo;
			if (initFirst(nodes[0]))
			{
				nodeListToAtTo = firstNode[position];
				nodeListToAtTo.Prepend(nodes[0]);
				if (position > firstNode.Lenght)
				{
					firstNode.Prepend(nodes[0]);
					return firstNode.Lenght - 1;
				...
				{
					firstNode[position].Prepend(nodes[0]);
					return unchecked((int)position);
				}
			}
			else nodeListToAtTo = firstNode;

			for (int i = 1; i < nodes.Length; i++)
			{
				nodeListToAtTo.Prepend(nodes[i]);
			}
		}
		...
		public virtual int Length
		{
			get
			{
				return firstNode == null ? 0 : firstNode.Lenght;
			}
		}
